...
class CowIngestor(BaseIngestor):
    ...
    def _get_owners(self) -> List[str]:
        ...
        logger.info(f"Found {len(owners)} owners to query ({self.mode} mode)")
        return owners
    ...
    def _get_existing_order_uids(self, owners: List[str]) -> Set[str]:
        ...
        try:
            ...
            logger.info(f"Found {len(uids)} existing order_uids to skip")
            return uids
        except Exception:
            return set()
        ...
        for attempt in range(MAX_RETRIES + 1):
            try:
                ...
                if attempt < MAX_RETRIES:
                    logger.warning(f"Retry {attempt + 1}/{MAX_RETRIES} for {url[:80]}... ({e})")
                    time.sleep(3)
                else:
    ...
    def _fetch_trades_for_owner(
        ...
        while page < max_pages:
            ...
            try:
                ...
                logger.error(f"API error for owner {owner}: {e}")
                break
        ...
        if page >= max_pages:
            logger.warning(f"Owner {owner} hit max pages ({max_pages}), fetched {len(all_trades)} trades")
    ...
    def ingest(self, skip_table_creation: bool = False, **kwargs) -> bool:
        ...
        try:
            ...
            for i, owner in enumerate(owners):
                if i > 0 and i % 50 == 0:
                    logger.info(f"Progress: {i}/{len(owners)} owners, {total_trades} new trades, {skipped_owners} skipped")
                ...
                logger.info(f"  [{i+1}/{len(owners)}] {owner[:10]}... → {len(trades)} new trades")
            ...
            logger.error(f"Error in CoW ingestion: {e}")
            return False
