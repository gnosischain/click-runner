...
INSERT INTO crawlers_data.celo_gpay_wallets (safe_address, owner_address, owners, issued_at, first_spend_at)
...
-- is_activated is deliberately NOT ingested here — it's 100% derivable from
-- first_spend_at (IS NOT NULL), so it's computed in int_celo_gpay_wallets.sql
-- instead of stored redundantly, matching how transfers stays unclassified
-- at ingestion.
SELECT
...
FROM url(
  ...
  'CSVWithNames',
  'safe_address String, owner_address String, owners_csv String, issued_at String, first_spend_at String'
);
